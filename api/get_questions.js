...
export default async function handler(req, res) {
    ...
    try {
        ...
        const queryText = `SELECT id, nama, pertanyaan, created_at FROM "gsa-aiforge_questions" ORDER BY created_at ASC`;
        const result = await client.query(queryText);
        res.status(200).json({ success: true, count: result.rowCount, data: result.rows });
    } catch (err) {
        console.error('Error fetching data:', err);
        res.status(500).json({ error: err.message || 'Terjadi kesalahan saat mengambil pertanyaan dari database.' });
    } finally {
        await client.end();
    }
}
